...
def Curve(start,end,result) :
    ...
    array_ab  = np.array([x2-x1, y2-y1])    
    array_ac  = np.array([px-x1,py-y1])
    array_oa  = np.array([x1,y1])
    array_ob  = np.array([x2,y2])
    ...
    BD=GetDistance(x2,y2,array_d[0],array_d[1])
    ...
    print (' BD:',BD)
    print (' BD/AB:',BD/AB)
    result = result.append({'word': str1,'C': array_oc, 'D': array_d,'CD':Max_distance,'CD/AB':Max_distance/AB,'BD/AB':BD/AB,'MCI':mci,'real_1':rl[1],'imag_1':im[1],'mod_1':mod[1],'real_2':rl[2],'imag_2':im[2],'mod_2':mod[2],'real_3':rl[3],'imag_3':im[3],'mod_3':mod[3]} ,ignore_index=True)
    return result

def GetCD(x1,y1,x2,y2,px,py):
    array_oa=np.array([x1,y1])
    array_ob=np.array([x2,y2])
    array_oc=np.array([px,py])
    d = np.linalg.norm(np.cross(array_ob-array_oa, array_oa-array_oc))/np.linalg.norm(array_ob-array_oa)
    return d
...
try:
    ...
    result=pd.DataFrame(columns=['word','C','D','CD','CD/AB','BD/AB','MCI','real_1','imag_1','mod_1','real_2','imag_2','mod_2','real_3','imag_3','mod_3'])
    column=float(data.shape[0])
    if column>0:
        times=int(column/100)
        start=0
        end=99
        i=0
        while i < times:
